    ...
    <div x-show="isOpen" @click.away="isOpen = false" style="display: none;" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-main border-tf rounded-lg w-full max-w-4xl">
            <form method="POST" action="{{ route('mistakes.store') }}" class="py-10 px-32 relative">
                ...
                <div class="mt-4">
                    <x-text-area name="text" rows="10" class="w-full border-tf bg-back rounded p-2 text-custom-EBE3CB placeholder-custom-EBE3CB" placeholder="Опишите ошибку..." required></x-text-area>
                </div>
                ...
                <div class="mt-7 flex justify-end gap-3">
                    <button type="submit" class="px-16 py-4 text-2xl mx-auto bg-front rounded border-tf hover:text-custom-text-hover">
                        Отправить
                    </button>
                </div>
            </form>
