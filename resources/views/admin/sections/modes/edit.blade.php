<x-admin-layout>
    <x-slot name="pageTitle">Редактировать игровой режим</x-slot>
    <div class="w-3/4 mx-auto mt-12 h-full block">
        ...
        <form action="{{ route('admin.modes.update', $mode->id) }}" method="POST">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block">Название режима</label>
                    <input type="text" name="title" value="{{ old('title', $mode->title) }}"
                ...
                <div>
                    ...
                    <x-text-area name="text" rows="10" required
                                 class="w-full px-3 py-2 rounded" value="{{ old('text', $mode->text) }}"></x-text-area>
                </div>
                <div class="flex space-x-2">
                    <x-primary-button type="submit">
                        Обновить
                    </x-primary-button>
                    <a href="{{ route('admin.modes') }}"
