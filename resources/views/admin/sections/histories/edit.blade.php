<x-admin-layout>
    <x-slot name="pageTitle">Редактировать историческую статью</x-slot>
    <div class="w-3/4 mx-auto mt-12 h-full block">

        <h1 class="border-bottom-EBE3CB text-2xl mb-4">
            {{ $history->title }} - Редактирование
        </h1>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <x-text-area name="text" rows="10" required
                                 class="w-full px-3 py-2  rounded" value="{{ old('text', $history->text) }}"></x-text-area>
                </div>
                <div class="flex space-x-2">
                    <x-primary-button type="submit">
                        Обновить
                    </x-primary-button>
                    <a href="{{ route('admin.histories') }}" class="hover:text-custom-text-hover text-lg text-center px-2 py-2">
                        Отмена
                    </a>
                </div>
            </div>
